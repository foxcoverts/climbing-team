@props(['alert'])
@php
    if (Arr::has($alert, 'info')) {
        $alert['message'] = $alert['info'];
        $alert['type'] = 'info';
    } elseif (Arr::has($alert, 'error')) {
        $alert['message'] = $alert['error'];
        $alert['type'] = 'error';
    }
    $color = match (Arr::get($alert, 'type', 'info')) {
        'error' => 'red',
        default => 'blue',
    };
@endphp
@if (Arr::has($alert, 'message'))
    <div {{ $attributes->class([
        'border-b mb-2 sm:mb-0 text-black px-4 sm:px-8 py-3 flex items-start',
        ...
        <p class="flex-grow" x-init="{{ Arr::has($alert, '$dispatch') ? '$dispatch(' . Js::from(Arr::get($alert, '$dispatch')) . ')' : '' }}">
            {{ Arr::get($alert, 'message') }}
            @if (Arr::has($alert, 'restore'))
                <form action="{{ Arr::get($alert, 'restore') }}" method="post">
                    @csrf
                    @method('PATCH')
