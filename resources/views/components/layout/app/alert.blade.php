@if (session('status'))
    <div class="bg-blue-100 border-blue-200 border-b mb-2 sm:mb-0 text-black px-4 py-3 flex items-center"
        :class="{ 'hidden': !open }" role="alert" x-data="{ open: true }">
...
@endif
@if ($errors->any())
    <div class="bg-red-100 border-red-200 border-b mb-2 sm:mb-0 text-black px-4 py-3 flex items-center"
        :class="{ 'hidden': !open }" role="alert" x-data="{ open: true }">
        <p class="text-sm flex-grow">
            {{ $errors->first() }}
        </p>
