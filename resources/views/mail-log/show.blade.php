            ...
            <div class="space-y-2 my-2 w-full flex-grow">
                <p><dfn class="block not-italic font-medium after:content-[':']">@lang('Sent')</dfn>
                    ...
                    @if ($mail->calendar)
                        ...
                        <p><dfn class="block not-italic font-medium after:content-[':']">@lang('Method')</dfn>
                            <x-text-input name="method" :value="$mail->calendar->getMethod()" class="w-full mt-1" />
                        </p>

                        @foreach ($mail->calendar->getEvents() as $event)
                            <p><dfn class="block not-italic font-medium after:content-[':']">@lang('Booking')</dfn>
                                ...
                                @else
                                    <x-text-input name="to" :value="$event->getUid()" class="w-full mt-1" readonly />
                                @endif
                            </p>

                            @foreach ($event->getAttendees() as $attendee)
                                <p><dfn class="block not-italic font-medium after:content-[':']">@lang('Attendee')</dfn>
                                    ...
                                    @else
                                        <x-text-input name="attendee[]" :value="$attendee->getEmail()" class="w-full mt-1"
                                            readonly />
                                    @endif
                                </p>

                                <p><dfn class="block not-italic font-medium after:content-[':']">@lang('Status')</dfn>
                                    <x-text-input name="attendee[][status]" :value="__('app.attendee.status.' . $attendee->getStatus()->value)" class="w-full mt-1"
                                        readonly />
                                </p>
                            @endforeach
                        @endforeach
