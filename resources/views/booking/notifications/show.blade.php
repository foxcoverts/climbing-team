...
<x-layout.app :title="__('Booking Settings')">
    <section x-data="{
        submitted: false,
        settings: {{ Js::from([
        ...
        @include('booking.partials.header', ['booking' => $settings->notifiable])

        <form method="post" action="{{ route('booking.notifications.update', $settings->notifiable) }}"
            id="update-settings" x-on:submit="setTimeout(() => submitted = true, 0)" class="p-4 sm:px-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm">
                        {{ __('Get an e-mail when this booking is cancelled.') }}
                    </p>
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm">
                        {{ __('Get an e-mail when a comment is made about this booking.') }}
                    </p>
                    <x-select-input id="comment_mail" name="comment_mail" class="mt-1 block"
                        x-model.fill="settings.comment_mail">
                        <option value="" selected>
                            {{ __('Inherit') }}
                            ({{ __('app.notification.comment-option.' . $inherited->comment_mail->value) }})
                        </option>
                        <hr>
        ...
        <form method="post" action="{{ route('booking.notifications.destroy', $settings->notifiable) }}"
            id="delete-settings">
            @csrf
            @method('delete')
        </form>
        ...
        <footer class="flex flex-wrap items-start gap-4 mt-2 mb-4 p-4 sm:px-8">
            ...
            <x-button.secondary :href="route('booking.show', $settings->notifiable)" :label="__('Back')" />
        </footer>
    </section>
</x-layout.app>
