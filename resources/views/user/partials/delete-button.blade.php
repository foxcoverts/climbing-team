@can('delete', $user)
    <x-button.danger x-data=""
        x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')">@lang('Delete')</x-button.danger>
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('user.destroy', $user) }}" class="p-6">
            ...
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                @lang('Are you sure you want to delete this account?')
            </h2>

            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                @lang('Once this account is deleted, all of its resources and data will be permanently deleted.')
            </p>

            <div class="mt-6">
                <x-input-label for="confirm">
                    @lang('Please type "DELETE" to confirm you would like to permanently delete this account.')
                </x-input-label>
