<x-layout.app :title="__('Update Document')">
    <section>
        ...
        <div class="p-4 sm:px-8">
            <form method="POST" action="{{ route('document.update', $document) }}" enctype="multipart/form-data"
                ...
                <div class="max-w-prose space-y-6">
                    <div class="space-y-1">
                        ...
                        <x-file-input id="file" name="file" class="block" accept=".pdf,application/pdf"
                    ...
                    <div>
                        ...
                        <x-text-input id="title" name="title" type="text" class="mt-1 block w-full"
                            maxlength="255" required x-model="form.title" autofocus />
                ...
                <footer class="mt-6 flex items-center gap-4">
                    <x-button.primary x-bind:disabled="submitted" :label="__('Update')"
                        x-text="submitted ? {{ Js::from(__('Please wait...')) }} : {{ Js::from(__('Update')) }}" />

                    @can('delete', $document)
                        <x-button.danger form="delete-document" x-bind:disabled="submitted" :label="__('Delete')" />
                    @endcan
            ...
            <form method="POST" action="{{ route('document.destroy', $document) }}" id="delete-document">
                @csrf @method('DELETE')
            </form>
        </div>
    </section>
